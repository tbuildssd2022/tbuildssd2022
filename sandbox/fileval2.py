...
#breaking into 3 seperate tests, ext, size, bytes
def allowed_file_ext(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

def allowed_file_size(filebytes):
    print("inside file size")
    fs=len(filebytes)
    print(fs)
    if (fs < 32000000):
        return True
...
def upload_file():
    if request.method == 'POST':
        ...
        if newfile.filename == '':
            ...
            return redirect(request.url)
        if newfile and allowed_file_ext(newfile.filename):
            flash('File uploaded')
        ...
        if allowed_file_size(upfilebytes):
            print("this file appears to be less than 32mb")
        else:
            print("this file is greater than 32mb")
            ###exit code here
